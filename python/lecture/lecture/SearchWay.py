...
def searchWay(Start=0, End=1000, Step=1):
    ...
    if vote == 'y' or vote == 'Y':
        ...
        searchNumber = 300
    print("#############")
    # from here (start , end , step , list , searchNumber)>> are ready
    ...
    def jump(listTest, stepj, searchNumber):
        timesearchjumpStart = time.time()
        startj = 0
        for i in range(stepj):
            startj = i
            for j in range(startj, len(listTest), stepj):
                if searchNumber == listTest[j]:
                    print(
